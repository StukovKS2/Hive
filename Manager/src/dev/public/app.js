...
(function() {
  ...
  const TRANSLATIONS = {
    en: {
      ...
      'tutorial.step2.title': 'Accounts',
      'tutorial.step2.body': 'The Accounts tab is where you prepare sessions and automation. From here you can:',
      'tutorial.step2.li1': '<strong>Manage accounts</strong> &mdash; Store credentials, server preferences, and proxy settings.',
      'tutorial.step2.li2': '<strong>Scripts</strong> &mdash; Use disk-based .js scripts for automation when Developer Mode is on.',
      'tutorial.step2.li3': '<strong>Accounts</strong> &mdash; Add email accounts to launch and manage them.',
      'tutorial.step2.dim': 'Start with accounts, then layer scripts once you are comfortable.',
      ...
      'home.conn.listening': 'Dashboard ready (MITM proxy removed)',
      'home.conn.clientDetected': 'Client connected',
      'home.conn.clientWaiting': 'Waiting for headless client...',
    ...
    es: {
      ...
      'settings.tab.developer': 'Desarrollador',
      ...
      'tutorial.step2.title': 'Cuentas',
      'tutorial.step2.body': 'La pestaña Cuentas sirve para preparar sesiones y automatizaciones.',
      'tutorial.step2.li1': '<strong>Gestionar cuentas</strong> &mdash; Guarda credenciales, servidores y ajustes de proxy.',
      'tutorial.step2.li2': '<strong>Scripts</strong> &mdash; Usa scripts .js en disco para automatización cuando el Modo desarrollador está activo.',
      'tutorial.step2.li3': '<strong>Cuentas</strong> &mdash; Añade cuentas de correo para iniciarlas y administrarlas.',
      'tutorial.step2.dim': 'Empieza con cuentas y añade scripts cuando te sientas cómodo.',
      ...
      'home.conn.listening': 'Panel listo (proxy MITM eliminado)',
      'home.conn.clientDetected': 'Cliente conectado',
      'home.conn.clientWaiting': 'Esperando cliente headless...',
    ...
    de: {
      ...
      'tutorial.step2.title': 'Konten',
      'tutorial.step2.body': 'Im Konten-Tab bereitest du Sitzungen und Automatisierungen vor.',
      'tutorial.step2.li1': '<strong>Konten verwalten</strong> &mdash; Speichere Zugangsdaten, Server und Proxy-Einstellungen.',
      'tutorial.step2.li2': '<strong>Skripte</strong> &mdash; Nutze .js-Skripte von der Festplatte, wenn der Entwicklermodus aktiv ist.',
      'tutorial.step2.li3': '<strong>Konten</strong> &mdash; Füge E-Mail-Konten zum Starten und Verwalten hinzu.',
      'tutorial.step2.dim': 'Beginne mit Konten und ergänze Skripte, sobald du dich sicher fühlst.',
      ...
      'home.conn.listening': 'Dashboard bereit (MITM-Proxy entfernt)',
      'home.conn.clientDetected': 'Client verbunden',
      'home.conn.clientWaiting': 'Warte auf Headless-Client...',
    ...
    pt: {
      ...
      'tutorial.step2.title': 'Contas',
      'tutorial.step2.body': 'A aba Contas é onde você prepara sessões e automações.',
      'tutorial.step2.li1': '<strong>Gerenciar contas</strong> &mdash; Salve credenciais, servidores e configurações de proxy.',
      'tutorial.step2.li2': '<strong>Scripts</strong> &mdash; Use scripts .js em disco para automação com o Modo desenvolvedor ativo.',
      'tutorial.step2.li3': '<strong>Contas</strong> &mdash; Adicione contas de e-mail para iniciar e gerenciar.',
      'tutorial.step2.dim': 'Comece pelas contas e adicione scripts quando estiver confortável.',
      ...
      'home.conn.listening': 'Painel pronto (proxy MITM removido)',
      'home.conn.clientDetected': 'Cliente conectado',
      'home.conn.clientWaiting': 'Aguardando cliente headless...',
    ...
    ja: {
      ...
      'tutorial.step2.title': 'アカウント',
      'tutorial.step2.body': 'アカウントタブでは、セッションと自動化の準備ができます。',
      'tutorial.step2.li1': '<strong>アカウント管理</strong> &mdash; 認証情報、サーバー、プロキシ設定を保存します。',
      'tutorial.step2.li2': '<strong>スクリプト</strong> &mdash; 開発者モードがオンのとき、ディスク上の .js スクリプトで自動化できます。',
      'tutorial.step2.li3': '<strong>アカウント</strong> &mdash; 起動および管理するメールアカウントを追加します。',
      'tutorial.step2.dim': 'まずはアカウントから始め、慣れたらスクリプトを追加しましょう。',
      ...
      'home.conn.listening': 'ダッシュボード準備完了（MITMプロキシ削除済み）',
      'home.conn.clientDetected': 'クライアント接続済み',
      'home.conn.clientWaiting': 'ヘッドレスクライアント待機中...',
  ...
  if (BACKEND_API_DISABLED) {
    ...
    dashboardUser = { email: 'local', is_admin: false, developer_mode: false };
    try {
  ...
  function refreshHeadlessAccountSelectors() {
    ['objects-account-select', 'nearby-account-select', 'tilemap-account-select', 'damage-account-select'].forEach(function (id) {
      ...
      select.innerHTML = '';
      headlessSessions.forEach(function (session, accountId) {
        var option = document.createElement('option');
        option.value = accountId;
        option.textContent = String(session.alias || session.email || accountId) + ' · ' + String(session.mapName || session.lifecycle || 'Connecting');
        select.appendChild(option);
      });
      if (previous && headlessSessions.has(previous)) select.value = previous;
      select.disabled = headlessSessions.size === 0;
  ...
  let homeMultiStatusFilter = ['all', 'ready', 'missing'].indexOf(homeMultiStatusFilterRaw) >= 0
  ...
  function applyAdminMode() {
    ...
    if (!adminMode) {
      if (activeTab === 'logs' || activeTab === 'packet-lab' || activeTab === 'market') {
        var fallbackBtn = document.querySelector('.content-tab[data-tab="accounts"]')
          || document.querySelector('.content-tab[data-tab="scripts"]');
  ...
  function renderNavbarTabSettings() {
    ...
    order.forEach(function (tabName) {
      ...
      cb.type = 'checkbox';
      cb.checked = !hidden;
      // Don't allow hiding the Clients tab
      if (tabName === 'clients') {
        cb.disabled = true;
        cb.checked = true;
      }
  ...
  function setActiveSettingsTab(tabName) {
    ...
    // Plugins settings panel removed — map stale localStorage values to visual
  ...
  // Settings is a transient popout: any click outside its dialog dismisses it.
  document.addEventListener('click', function (e) {
    if (!settingsPopout || !settingsPopout.classList.contains('settings-popout-open')) return;
    if (e.target.closest('.settings-popout-panel, #btn-settings')) return;
    closeSettingsPopout();
  }, true);
  ...
  (function relocateSettingsIntoTab() {
  ...
  function tryRefreshToken() {
    if (BACKEND_API_DISABLED) return Promise.resolve(false);
  ...
  function fetchCurrentUser(allowRefreshRetry) {
    if (BACKEND_API_DISABLED) {
      ...
      dashboardUser = { email: 'local', is_admin: false, developer_mode: false };
      updateDashboardAvailabilityUi();
      return Promise.resolve(dashboardUser);
    }
    if (!accessToken) {
  ...
  function restoreDashboardSessionFromTokens() {
    if (BACKEND_API_DISABLED) return Promise.resolve(true);
  ...
  /**
   * Product analytics removed — kept as a no-op so existing call sites stay safe.
   */
  function trackEvent(_eventName, _props) {}
  window._trackEvent = trackEvent;

  var splashDismissed = false;
  function updateDashboardAvailabilityUi() {
    if (BACKEND_API_DISABLED) dashboardLoggedIn = true;
  ...
  function renderHomeTab() {
    ...
    // Clients tab always shows the MAC multi-account overview.
    ...
    connectedLayout.classList.add('hidden');
    if (disconnectedLayout) disconnectedLayout.classList.add('hidden');
    if (multiPlaceholder) {
      multiPlaceholder.classList.add('hidden');
      multiPlaceholder.setAttribute('aria-hidden', 'true');
    }
    if (homeToolbar) homeToolbar.classList.add('hidden');
    if (multiLayout) multiLayout.classList.remove('hidden');
    renderMultiHomeTab();
  }
  ...
  function getConnectedClientSnapshot(clientId, c) {
    ...
    return {
      ...
      charIdDisplay: pd.characterId != null ? String(pd.characterId) : '--',
      pos: pd.pos || null,
      hp: hp,
      maxHp: maxHp,
  ...
  function patchMultiHomeConnectedCardDom(clientId) {
    ...
    if (!grid || !isMacMultiHome() || activeTab !== 'clients' || !connectedClients.has(clientId)) return false;
  ...
  function renderMultiHomeConnectedCards() {
    ...
    clients.forEach(function (entry) {
      ...
      card.setAttribute('aria-label', 'Open details for ' + snap.name);
      ...
      switch (msg.type) {
        ...
        case 'clientList': {
          var previousLiveClientIds = Array.from(connectedClients.keys()).filter(function (clientId) {
            return clientId !== SINGLE_ACCOUNT_CLIENT_ID;
          });
          var liveClientIds = new Set();
          (Array.isArray(msg.clients) ? msg.clients : []).forEach(function (client) {
            var clientId = String(client && (client.clientId || client.id) || '');
            if (!clientId) return;
            liveClientIds.add(clientId);
            connectedClients.set(clientId, client);
            if (!connectedClientFirstSeenAt.has(clientId)) {
              connectedClientFirstSeenAt.set(clientId, Date.now());
            }
          });
          Array.from(connectedClients.keys()).forEach(function (clientId) {
            if (clientId === SINGLE_ACCOUNT_CLIENT_ID) return;
            if (!liveClientIds.has(clientId)) {
              connectedClients.delete(clientId);
              connectedClientFirstSeenAt.delete(clientId);
              if (macPopoutOpenClientId === clientId) closeMacPopout();
            }
          });
          var clientStructureChanged = previousLiveClientIds.length !== liveClientIds.size ||
            previousLiveClientIds.some(function (clientId) { return !liveClientIds.has(clientId); });
          if (activeTab === 'clients') {
            var liveGrid = document.getElementById('home-multi-connected-cards-grid');
          ...
          if (isMacStyleSidebar() && multiAccountSidebarMode === 'connected') {
  ...
  function renderTilemapTree(data) {
    ...
    groups.forEach(function (group) {
      ...
      var tileName = String(group.name || ('0x' + Number(group.tileType || 0).toString(16)));
      if (/^SNR\b/i.test(tileName)) tileName = 'Summer Nexus (' + tileName + ')';
      label.textContent = tileName + ' · ' + String(group.tileType || 0) + '  [' + ((group.tiles || []).length) + ']';
  ...
  function requestNearbyPlayersOnce() {
    if (ws && ws.readyState === 1) {
      ws.send(JSON.stringify({ type: 'requestNearbyPlayers', accountId: nearbyAccountSelect && nearbyAccountSelect.value || null }));
    }
  }

  // Headless client cards receive frequent live patches. Delegate pointer
  // activation to the stable document so a card refresh cannot swallow it.
  document.addEventListener('pointerdown', function (e) {
    if (e.button != null && e.button !== 0) return;
    var card = e.target && e.target.closest ? e.target.closest('.home-multi-live-card') : null;
    if (!card || e.target.closest('.home-multi-live-controls')) return;
    var clientId = String(card.getAttribute('data-multi-live-client-id') || '');
    if (!clientId || !connectedClients.has(clientId)) return;
    e.preventDefault();
    card.focus();
    multiHomeFocusedClientId = clientId;
    openMacPopout(clientId);
  ...
  function renderNearbyPlayersTab() {
    ...
    for (const p of players) {
      ...
      tr.addEventListener('contextmenu', function (e) {
        ...
        if (ws && ws.readyState === 1) {
          ws.send(JSON.stringify({
            type: 'requestNearbyPlayerDebug',
            objectId: p.objectId,
            accountId: nearbyAccountSelect && nearbyAccountSelect.value || null,
          }));
        }
      });
  ...
  document.getElementById('content-tabs').addEventListener('click', (e) => {
    ...
    if (tabName === 'clients') renderHomeTab();
  ...
  const objectsAccountSelect = document.getElementById('objects-account-select');
  ...
  const tilemapAccountSelect = document.getElementById('tilemap-account-select');
  ...
  function requestObjects() {
    if (ws && ws.readyState === 1) {
      ws.send(JSON.stringify({ type: 'requestObjects', accountId: objectsAccountSelect && objectsAccountSelect.value || null }));
    }
  }

  function requestTilemap() {
    if (ws && ws.readyState === 1) {
      ws.send(JSON.stringify({ type: 'requestTilemap', radius: 12, accountId: tilemapAccountSelect && tilemapAccountSelect.value || null }));
    }
  }
  ...
  function setAccountsDirty(nextDirty, statusText) {
    ...
    if (accountsSaveBtn) {
      accountsSaveBtn.textContent = accountsDirty ? 'Save & Verify *' : 'Save & Verify';
  ...
  function isPotionOverviewItem(item) {
    return /\bpotion\b/i.test(getEquipmentItemLabel(item));
  }

  // Vault and gift storage can contain repeated potion slots. Keep every
  // non-potion item distinct, but collapse identical potions into one stack.
  function stackPotionsInStorageItems(items) {
    var stacks = Object.create(null);
    var result = [];
    (Array.isArray(items) ? items : []).forEach(function (item) {
      if (!isPotionOverviewItem(item)) {
        result.push(item);
        return;
      }
      var objectType = Number(item && item.objectType);
      if (!Number.isFinite(objectType) || objectType < 0) {
        result.push(item);
        return;
      }
      var key = String(objectType);
      if (!stacks[key]) {
        stacks[key] = { item: item, count: 0 };
        result.push(stacks[key]);
      }
      stacks[key].count += 1;
    });
    return result;
  ...
  function renderAccountsOverview() {
    ...
    if (accountsOverviewRefreshBtn) {
      ...
      accountsOverviewRefreshBtn.textContent = loading ? 'Loading...' : 'Verify & Refresh';
    }
    ...
    if (!overview) {
      ...
      accountsCharactersListEl.innerHTML = '';
      accountsCharactersEmptyEl.style.display = '';
      accountsCharactersEmptyEl.textContent = loading ? 'Loading characters...' : 'Click Verify & Load Account on the Credentials tab, or Verify & Refresh here.';
      accountsCharacterDetailEl.innerHTML = '<div class="accounts-character-placeholder">' + (loading ? 'Fetching character data from RotMG...' : 'Verify the credentials to load characters, vault, gifts, and potions.') + '</div>';
      return;
    }
  ...
  function renderAccountsOverview() {
    ...
    if (selectedAccountsOverviewTab !== 'characters') {
      ...
      if (selectedAccountsOverviewTab === 'totals') {
        ...
        if (selectedAccountsOverviewTab === 'vault' || selectedAccountsOverviewTab === 'gifts' || selectedAccountsOverviewTab === 'potions') {
          browserItems = stackPotionsInStorageItems(browserItems);
        }
  ...
  function setAccountsVerifyStatus(text, isError) {
    if (!accountsVerifyStatusEl) return;
    accountsVerifyStatusEl.textContent = String(text || '');
    accountsVerifyStatusEl.classList.toggle('error', !!isError);
  }

  function verifySelectedDashboardAccount() {
    var account = getSelectedDashboardAccount();
    if (!account) {
      setAccountsVerifyStatus('Select or add an account first.', true);
      return;
    }
    updateSelectedDashboardAccountFromEditor();
    account = getSelectedDashboardAccount();
    var email = String(account && account.email || '').trim();
    var password = String(account && account.password || '');
    if (!email || !password) {
      setAccountsVerifyStatus('Email and password are required.', true);
      return;
    }
    if (accountsSaveBtn) accountsSaveBtn.disabled = true;
    setAccountsVerifyStatus('Saving and verifying with RotMG...', false);
    saveDashboardAccounts(true, true)
      .then(function () {
        return loadSelectedDashboardAccountOverview(true);
      })
      .then(function (overview) {
        if (!overview) {
          setAccountsVerifyStatus('Verification failed. Check the Overview message.', true);
          switchAccountsEditorTab('overview');
          return;
        }
        var charCount = Array.isArray(overview.characters) ? overview.characters.length : 0;
        setAccountsVerifyStatus('Verified. Loaded ' + charCount + ' character' + (charCount === 1 ? '' : 's') + '.', false);
        switchAccountsEditorTab('overview');
      })
      .catch(function (err) {
        setAccountsVerifyStatus(String(err && err.message || 'Verification failed.'), true);
      })
      .finally(function () {
        if (accountsSaveBtn) accountsSaveBtn.disabled = !accountsDirty;
      });
  }
  ...
  function renderAccountsEditor() {
    ...
    if (displaySubEl) displaySubEl.textContent = account && showAccountEmails ? (account.email || '') : '';
    if (avatarEl) {
  ...
  function loadDashboardAccounts() {
    return fetch('/api/accounts')
      ...
      .then(function (data) {
        ...
        if (selectedAccountId && accountOverviewById[selectedAccountId]) {
          switchAccountsEditorTab('overview');
        }
  ...
  function launchGameWithCredentials(email, password, serverName, source, launchOpts) {
    ...
    if (!email || !password) {
      setAccountsStatus('Email and password are required to launch.', true);
      return false;
    }
    if (!ws || ws.readyState !== 1) {
  ...
  if (accountsSaveBtn) {
    accountsSaveBtn.addEventListener('click', function () {
      verifySelectedDashboardAccount();
    });
  }
  ...
  //   Step 1: pick method (email / import)
  ...
  (function () {
    ...
    var setupMethod = null;  // 'email' | 'import'
    ...
    function showStep2(method) {
      ...
      var labelMap = { email: 'Email & Password', import: 'Import existing' };
    ...
    document.querySelectorAll('.setup-method-card').forEach(function (card) {
      card.addEventListener('click', function () {
        ...
        if (m === 'email' || m === 'import') showStep2(m);
      });
    });
    ...
    // ── Import from Launcher (email accounts only) ─────────────────────────
    ...
    function wireImportLauncher(btnId, statusId) {
      ...
      btn.addEventListener('click', async function () {
        ...
        try {
          ...
          var guid = String(result.guid || '').trim();
          if (!guid || guid.indexOf('@') < 0) {
            setImportStatusIn(statusId, 'Launcher is not signed into an email account.', true);
            return;
          }
          var next = createEmptyDashboardAccount();
          next.email = guid;
    ...
    wireImportLauncher('accounts-setup-import-launcher-btn-2', 'accounts-setup-import-status-2');

    // ── Add Account button (email variant) ─────────────────────────────────
    if (setupAddBtn) {
      setupAddBtn.addEventListener('click', function () {
        if (!setupMethod || setupMethod !== 'email') return;
        ...
        if (!email || !password) {
          setAccountsStatus('Email and password are required.', true);
          return;
        }
        var next = createEmptyDashboardAccount();
  ...
  function updateSelectedDashboardAccountFromEditor() {
    ...
    if (ds2) ds2.textContent = showAccountEmails ? (account.email || '') : '';
    if (av2) av2.textContent = (account.label || account.email || 'A').charAt(0).toUpperCase();
  }

  [accountsAliasInput, accountsEmailInput, accountsPasswordInput, accountsNotesInput].forEach(function (el) {
    if (!el) return;
  ...
  if (accountsImportLauncherBtn) {
    accountsImportLauncherBtn.addEventListener('click', async function () {
      ...
      try {
        ...
        var guid = String(result.guid || '').trim();
        if (!guid || guid.indexOf('@') < 0) {
          setImportLauncherStatus('Launcher is not signed into an email account.', true);
          return;
        }
        if (accountsEmailInput) accountsEmailInput.value = guid;
        if (result.secret && accountsPasswordInput) accountsPasswordInput.value = result.secret;
        updateSelectedDashboardAccountFromEditor();
        var bits = [];
        bits.push('Email: ' + guid);
        if (result.secret) bits.push('Password: ✓ (' + result.secret.length + ' chars)');
  ...
  // Proxy credential inputs
  [accountsProxyUsername, accountsProxyPassword].forEach(function (el) {
    if (!el) return;
    el.addEventListener('input', updateSelectedDashboardAccountFromEditor);
    el.addEventListener('change', updateSelectedDashboardAccountFromEditor);
  });
  ...
  function renderScriptsListFromDataLegacy(data) {
    ...
    if (!scripts.length) {
      ...
      empty.textContent = 'No script packages found. Add a folder with hive.script.json and a .mjs entry.';
  ...
  function renderScriptsListFromData(data) {
    ...
    if (!scripts.length) {
      listEl.innerHTML = '<div class="plugin-sidebar-empty">No script packages found. Add a folder with hive.script.json and a .mjs entry.</div>';
  ...
  (function initTutorial() {
    var TUTORIAL_KEY = 'hive_tutorial_done';
    ...
    var stepTabs = [null, 'clients', 'accounts', 'accounts', 'damage', null];
  ...
  function initHomeExtras() {
    ...
    if (pluginsBtn) pluginsBtn.addEventListener('click', function () {
      var t = document.querySelector('[data-tab="scripts"]')
        || document.querySelector('[data-tab="accounts"]');
      if (t) t.click();
    });
    if (upgradeBtn) upgradeBtn.addEventListener('click', function () {
      var t = document.querySelector('[data-tab="market"]');
